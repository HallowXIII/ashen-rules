...
#let flavor(body) = {
  set text(font: ("Libre Baskerville",), style: "italic")
  box[#body]
}
...
#let power-description(
  ...
  #if flavor != none [_#flavor \ _]
  #if prerequisites != none { for req in prerequisites {
...
#let pftab(name, columns: (1fr, 4fr), breakable: false, ..contents) = [
  ...
  *#smallcaps(text(size: 1.3em)[#upper(name)])*
  ...
  fill: (col, row) => if row == 0 { colors.pfnavy } else if calc.odd(row+1) { colors.pfwhite } else { colors.otherRow },
  inset: 5pt,
  stroke: none,
  // align: horizon,
  ..contents
